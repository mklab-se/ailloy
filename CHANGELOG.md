...
- Image generation no longer fails with `missing field input_tokens` on models that report
  usage with a different vocabulary than gpt-image. MAI-Image-2.x on Microsoft Foundry
  returns `num_input_text_tokens` / `num_input_image_tokens` / `num_output_tokens` (and no
  total); these are now mapped to prompt/completion/total tokens, and an unrecognized
  `usage` shape yields no usage instead of discarding the generated image.

### Changed

- Documented MAI-Image as a supported Microsoft Foundry image deployment alongside
  gpt-image-2 (README.md, the `ai-reference` skill doc).
...
- Config TUI: the `[ Discover via Azure CLI ]` action in the add-node form now
  sits directly beneath the provider selector (it prefills the fields below
  it), and is worded "Azure CLI" instead of "az CLI"

## [2.0.3] - 2026-07-16

### Fixed

- `--help` polish: example blocks use comment-above style instead of inline
  trailing notes, global flags (`-v`, `-q`, `--no-color`) always render after
  command-specific flags, and flag descriptions no longer repeat the values
  clap already lists under `[possible values: ...]`
