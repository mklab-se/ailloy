...
#[command(after_help = "\
Examples:
  # Gate a script or test on AI judgment: exit 0 = pass, 1 = fail
  my-tool run | ailloy eval -c \"output mentions the order id\"

  # Score against a threshold and print the verdict as JSON
  ailloy eval \"$output\" -c \"polite, professional English\" --threshold 0.8 --json

  # Criteria and input from files, with context for the judge
