...
src/
  ...
  types.rs            # Message, MessageContent (Text/Parts, untagged), ContentPart
                      ...
                      #   response_format), ResponseFormat (JsonObject/JsonSchema +
                      #   per-provider request values), StreamEvent, ChatStream,
                      #   ImageResponse, ImageOptions (incl. output_format/compression/
  ...
  video_jobs.rs       # Shared Azure/Foundry video-jobs (sora-2) request logic (private) — URL
                      #   building (v1 surface appends ?api-version=preview; dated api_version
                      #   nodes use the legacy path instead), create/get/download/delete wire types
...
- **Video generation**: `Capability::Video`/`Task::VideoGeneration` (config key `video`), implemented only for Azure OpenAI and Microsoft Foundry via the shared jobs API in `video_jobs.rs` (`POST/GET/DELETE .../video/generations/jobs...`). Follows the same v1-vs-dated endpoint rule as chat/images: no `api_version` on the node → v1 surface with `?api-version=preview` appended to job endpoints; explicit `api_version` → legacy dated endpoints. `Provider::generate_video` (default method) creates a job, polls `get_video_job` (2s→10s backoff, 15-min timeout), and downloads every generation via `download_video`; other providers return `Unsupported`
